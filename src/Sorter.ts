...
export class Sorter<T> implements ISorter<T> {
  ...
  public sortByFunction(
    comparisonFunction: ISortInterFace<T>,
    data: T[],
    lowIndex: number,
    highIndex: number
  ): void {
  ...
  public sortByImplementation(
    ...
    this.internalSort(
      ...
      lowIndex,
      highIndex,
      SorterType.sortByImplementation
    );
  }
   ...
   * @param highIndex Higher index within the list to check
   */
  public sortTupleByFunction(
    comparisonFunction: ISortInterFace<T>,
    data: KeyValuePair<T, any>[],
    lowIndex: number,
    highIndex: number
  ): void {
    this.internalTupleSort(
      data,
      lowIndex,
      highIndex,
      SorterType.sortByFunction,
   ...
   * @param highIndex Higher index within the list to check
   */
  public sortTupleByImplementation(
    data: KeyValuePair<T, any>[],
    lowIndex: number,
    highIndex: number
  ): void {
  ...
  private internalSort(
    ...
    for (let i: number = lowIndex; i < highIndex - 1; i++) {
      if (
        type === SorterType.sortByFunction &&
